...
interface OneDriveFolderPickerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectFolder: (folderId: string, folderName: string) => void;
}

export function OneDriveFolderPickerModal({
  isOpen,
  onClose,
  onSelectFolder,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setBreadcrumbs([{ id: 'root', name: 'OneDrive (ルート)' }]);
      fetchFolders('root');
    }
  }, [isOpen]);
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {folders.map((folder) => (
                ...
                >
                  <Folder className="h-5 w-5 text-blue-500 shrink-0 group-hover:scale-110 transition-transform" />
